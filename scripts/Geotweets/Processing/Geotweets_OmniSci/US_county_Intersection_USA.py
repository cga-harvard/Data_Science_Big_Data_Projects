...
import glob
from os.path import isfile, join# path = '/n/holyscratch01/cga/dkakkar/data/geotweets/2020/' # use your path

import pyarrow as pa;import numpy as np
from datetime import datetime,date
from tqdm import tqdm

# make a test in nerc now?

# why must be csv files, lol
...
def sjoin_process(input_path,output_path,port,start_date,end_date):
    ...
    for filename in tqdm(need_process):
    ...
    #print(l_ni)
    #  break
    now = datetime.now().isoformat().replace(":","_").split(".")[0]
    with open(now +'_logfile.txt', 'w') as filehandle:
