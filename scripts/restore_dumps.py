...
all_files = glob.glob(path + "/archive_backup_2014*")
for filename in all_files:
    print(filename)
    #archive_name="/n/cga/geotweets_merged/" + filename
    #print (archive_name)
    cmd= "pg_restore  --host localhost --port 10320 -U postgres -d postgres --section=pre-data --section=data -1 "+ filename
    #print(cmd)
    os.system(cmd)
    TBL= 'archive'+'.'+'twitter'+'_'+filename.split('_')[3]+'_'+filename.split('_')[4]
    #print(TBL)
    pwd='/n/holyscratch01/cga/dkakkar/data/geotweets/results/2014/input/'+'twitter'+'_'+filename.split('_')[3]+'_'+filename.split('_')[4]+'.csv'
    pwd= "'{}'".format(pwd)
    #print(pwd)
    try:
      conn = psycopg2.connect("dbname='postgres' user='postgres' host='localhost' port='10320' password='1234'")
    except:
